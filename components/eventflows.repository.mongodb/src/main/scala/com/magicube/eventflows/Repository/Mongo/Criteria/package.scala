...
package object Criteria {
  ...
  implicit def toBSONDocument(expr: Expression): BSONDocument = expr match {
    case Expression(Some(name), BSONElement(field, element)) if (name == field) =>
      BSONDocument(field -> element)

    case Expression(Some(name), element) => BSONDocument(name -> BSONDocument(element));

    case Expression(None, BSONElement("", _)) => BSONDocument.empty
  ...
  implicit def bsonValueIdentityValue[T <: BSONValue]: ValueBuilder[T] = new ValueBuilder[T] {
